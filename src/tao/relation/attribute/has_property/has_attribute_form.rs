...
mod tests {
    ...
    fn check_type_attributes() {
        initialize_kb();
        #[rustfmt::skip]
        assert_eq!(HasAttribute::archetype().introduced_attribute_archetypes(), vec![]);
        #[rustfmt::skip]
        assert_eq!(HasAttribute::archetype().attribute_archetypes(), vec![Owner::archetype(), Value::archetype()]);
    }
    ...
    fn from_name() {
        ...
        #[rustfmt::skip]
        assert_eq!(HasAttribute::try_from("A").map(|c| c.id()), Ok(concept.id()));
